...
const FireMap: React.FC<FireMapProps> = ({ fireCollection, fullCollection, onSearch, dates, currentDate, onDateChange, searchParams, initialSettings, onSettingsChange, onViewportChange, onQuickRange }) => {
  ...
  }, [showHeatmap, showCluster, showStats, showTrends, showRadar, baseLayer, showGraticule, showCountryOutline, showStreetsRef, weightBy, threshold, filterEnabled, filterBy, filterThreshold, filterAffectsAnalytics, onSettingsChange]);
  ...
  const MeasureLayerPro: React.FC<{
    ...
    React.useEffect(() => {
      ...
      const formatArea = (m2: number) => {
        if (areaUnit === 'km2') return `${(m2 / 1e6).toFixed(2)} km虏`;
        if (areaUnit === 'mi2') return `${(m2 / 1e6 * 0.386102).toFixed(2)} mi虏`;
      ...
      const clearAll = () => {
        ...
        segTipsRef.current = [];
        if (tipsRef.current) { layer.removeLayer(tipsRef.current); tipsRef.current = null; }
        if (cursorLineRef.current) { layer.removeLayer(cursorLineRef.current); cursorLineRef.current = null; }
        if (cursorTipRef.current) { layer.removeLayer(cursorTipRef.current); cursorTipRef.current = null; }
      ...
      const redraw = () => {
        ...
        if (mode === 'distance') {
          ...
          // draw vertex markers
          vertexRef.current.forEach(m => layer.removeLayer(m));
          vertexRef.current = pointsRef.current.map(p => L.circleMarker(p, { radius: 4, color: '#22c55e', weight: 2, fillColor: '#0f172a', fillOpacity: 0.9 }).addTo(layer));
          ...
          if (lineRef.current) { layer.removeLayer(lineRef.current); lineRef.current = null; }
          if (!polyRef.current) polyRef.current = L.polygon([], { color: '#22c55e', weight: 2, fillOpacity: 0.1 }).addTo(layer);
      ...
      const onClick = (e: L.LeafletMouseEvent) => {
        ...
        if (layerRef.current) {
          const v = L.circleMarker(e.latlng, { radius: 4, color: '#22c55e', weight: 2, fillColor: '#0f172a', fillOpacity: 0.9 });
      ...
      const onDblClick = () => {
        if (!enabled) return;
        // finalize current measurement into saved layer
        if (!savedRef.current) savedRef.current = L.layerGroup().addTo(map);
        const saved = savedRef.current;
        const pts = pointsRef.current.slice();
        if ((mode === 'distance' && pts.length >= 2) || (mode === 'area' && pts.length >= 3)) {
          if (mode === 'distance') {
            const pl = L.polyline(pts, { color: '#22c55e', weight: 3 }).addTo(saved);
            // vertex markers
            pts.forEach(p => L.circleMarker(p, { radius: 4, color: '#22c55e', weight: 2, fillColor: '#0f172a', fillOpacity: 0.9 }).addTo(saved));
            // per-segment tips
            ...
            const pg = L.polygon(pts, { color: '#22c55e', weight: 2, fillOpacity: 0.1 }).addTo(saved);
            // vertex markers
            pts.forEach(p => L.circleMarker(p, { radius: 4, color: '#22c55e', weight: 2, fillColor: '#0f172a', fillOpacity: 0.9 }).addTo(saved));
      ...
      const onMove = (e: L.LeafletMouseEvent) => {
        ...
        const curr = e.latlng;
        if (mode === 'distance') {
          if (!cursorLineRef.current) cursorLineRef.current = L.polyline([last, curr], { color: '#22c55e', weight: 2, dashArray: '4 4' }).addTo(layer);
          cursorLineRef.current.setLatLngs([last, curr]);
          const lenKm = haversine(last, curr);
          ...
          cursorTipRef.current.setLatLng(curr).setContent(kmTo(lenKm));
